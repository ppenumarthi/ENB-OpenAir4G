...
static void send_msg_added()
{
	...
	/* NFVEPC 
	 * Generate traffic only when we should generate traffic.
	 * This is visible in file
	 * 0 means no traffic 
	 * 1 means 1 packet is transmitted
	 */
	FILE *f;
	if(execNum >1 )
		f = fopen(secondFile, "rb");
	else
		f = fopen(firstFile, "rb");
	int first = fgetc(f);
	fclose(f);

	if(first == 48) // verify for ASCII of '0'
		return;
...
*/	
	...
	while(count < value)
	{
		if( send(sock , (void *) msg_p, sizeof(MessageDef) , 0) < 0)
		{
